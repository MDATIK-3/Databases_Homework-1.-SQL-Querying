WITH
  ...
  (
    ...
    FROM EmployeePayHistory p
      JOIN (
    SELECT BusinessEntityID, MAX(RateChangeDate) AS latest_date
      FROM EmployeePayHistory
      GROUP BY BusinessEntityID
  ) latest
      ON  latest.BusinessEntityID = p.BusinessEntityID
        AND latest.latest_date      = p.RateChangeDate
  )

-- Q1: Individual-level dataset (useful for scatter plot / regression analysis)
SELECT
...
FROM Employee e
  JOIN current_pay cp
  ON cp.BusinessEntityID = e.BusinessEntityID;

-- Q2: Summary – average pay by demographic group
WITH
  current_pay
  AS
  (
